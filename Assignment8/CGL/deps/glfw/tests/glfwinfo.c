...
static void usage(void) {
  printf("Usage: glfwinfo [-h] [-a API] [-m MAJOR] [-n MINOR] [-d] [-l] [-f] "
         "[-p PROFILE] [-s STRATEGY] [-b BEHAVIOR]\n");
  printf("Options:\n");
  printf("  -a the client API to use (" API_OPENGL " or " API_OPENGL_ES ")\n");
  printf("  -b the release behavior to use (" BEHAVIOR_NAME_NONE
         " or " BEHAVIOR_NAME_FLUSH ")\n");
  ...
  printf("  -p the OpenGL profile to use (" PROFILE_NAME_CORE
         " or " PROFILE_NAME_COMPAT ")\n");
  printf("  -s the robustness strategy to use (" STRATEGY_NAME_NONE
         " or " STRATEGY_NAME_LOSE ")\n");
}
...
int main(int argc, char **argv) {
  int ch, api = 0, profile = 0, strategy = 0, behavior = 0, major = 1,
          minor = 0, revision;
  ...
  printf("%s context version parsed by GLFW: %u.%u.%u\n",
         get_client_api_name(api), major, minor, revision);
  ...
  if (api == GLFW_OPENGL_API) {
    ...
    if (glfwExtensionSupported("GL_ARB_robustness")) {
      ...
      printf("%s robustness strategy parsed by GLFW: %s\n",
             get_client_api_name(api), get_strategy_name_glfw(robustness));
    }
  }
