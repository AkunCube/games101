...
static void draw_joystick(Joystick *j, int x, int y, int width, int height) {
  ...
  if (j->axis_count) {
    ...
    for (i = 0; i < j->axis_count; i++) {
      ...
      glRecti(x + i * axis_width, y + (int)(value * (axis_height - 5)),
              x + (i + 1) * axis_width,
  ...
  if (j->button_count) {
    ...
    for (i = 0; i < j->button_count; i++) {
      ...
      glRecti(x + i * button_width, y + axis_height, x + (i + 1) * button_width,
...
static void draw_joysticks(GLFWwindow *window) {
  ...
  for (i = 0; i < sizeof(joysticks) / sizeof(Joystick); i++) {
    ...
    if (j->present) {
      draw_joystick(j, 0, offset * height / joystick_count, width,
                    height / joystick_count);
      offset++;
    }
  }
}
