...
static void window_iconify_callback(GLFWwindow *window, int iconified) {
  ...
  printf("%08x to %i at %0.3f: Window was %s\n", counter++, slot->number,
         glfwGetTime(), iconified ? "iconified" : "restored");
}
...
static void key_callback(GLFWwindow *window, int key, int scancode, int action,
  ...
  printf(
      ...
      counter++, slot->number, glfwGetTime(), key, scancode, get_key_name(key),
...
static void char_callback(GLFWwindow *window, unsigned int codepoint) {
  ...
  printf("%08x to %i at %0.3f: Character 0x%08x (%s) input\n", counter++,
         slot->number, glfwGetTime(), codepoint,
...
static void char_mods_callback(GLFWwindow *window, unsigned int codepoint,
                               int mods) {
  Slot *slot = glfwGetWindowUserPointer(window);
  printf("%08x to %i at %0.3f: Character 0x%08x (%s) with modifiers (with%s) "
         "input\n",
         counter++, slot->number, glfwGetTime(), codepoint,
         get_character_string(codepoint), get_mods_name(mods));
}
