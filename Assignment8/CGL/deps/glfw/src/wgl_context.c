...
static void initWGLExtensions(_GLFWwindow *window) {
  ...
  if (_glfwPlatformExtensionSupported("WGL_ARB_pixel_format")) {
    window->wgl.GetPixelFormatAttribivARB =
...
//
static int getPixelFormatAttrib(_GLFWwindow *window, int pixelFormat,
                                int attrib) {
  int value = 0;

  if (!window->wgl.GetPixelFormatAttribivARB(window->wgl.dc, pixelFormat, 0, 1,
                                             &attrib, &value)) {
...
static GLboolean choosePixelFormat(_GLFWwindow *window,
                                   const _GLFWfbconfig *desired, int *result) {
  ...
  for (i = 0; i < nativeCount; i++) {
    ...
    if (window->wgl.ARB_pixel_format) {
      ...
      u->accumGreenBits =
          getPixelFormatAttrib(window, n, WGL_ACCUM_GREEN_BITS_ARB);
      u->accumBlueBits =
          getPixelFormatAttrib(window, n, WGL_ACCUM_BLUE_BITS_ARB);
      u->accumAlphaBits =
          getPixelFormatAttrib(window, n, WGL_ACCUM_ALPHA_BITS_ARB);
      ...
      if (!DescribePixelFormat(window->wgl.dc, n, sizeof(PIXELFORMATDESCRIPTOR),
                               &pfd)) {
        continue;
      }
...
int _glfwCreateContext(_GLFWwindow *window, const _GLFWctxconfig *ctxconfig,
  ...
  if (window->wgl.ARB_create_context) {
    ...
    window->wgl.context =
        window->wgl.CreateContextAttribsARB(window->wgl.dc, share, attribs);
