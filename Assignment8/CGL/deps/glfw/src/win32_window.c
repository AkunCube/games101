...
static GLboolean enterFullscreenMode(_GLFWwindow *window) {
  ...
  SetWindowPos(window->win32.handle, HWND_TOPMOST, xpos, ypos, mode.width,
               mode.height, SWP_NOCOPYBITS);
  ...
  switch (uMsg) {
    ...
    case WM_SYSKEYUP: {
      ...
      if (wParam == VK_SHIFT) {
        ...
        _glfwInputKey(window, GLFW_KEY_LEFT_SHIFT, scancode, GLFW_RELEASE,
                      mods);
        _glfwInputKey(window, GLFW_KEY_RIGHT_SHIFT, scancode, GLFW_RELEASE,
                      mods);
    ...
    case WM_MOUSEMOVE: {
      ...
      if (window->cursorMode == GLFW_CURSOR_DISABLED) {
        ...
        _glfwInputCursorMotion(window, x - window->win32.cursorPosX,
...
//
static void getFullWindowSize(_GLFWwindow *window, int clientWidth,
                              int clientHeight, int *fullWidth,
                              int *fullHeight) {
  RECT rect = {0, 0, clientWidth, clientHeight};
  AdjustWindowRectEx(&rect, window->win32.dwStyle, FALSE,
                     window->win32.dwExStyle);
...
static int createWindow(_GLFWwindow *window, const _GLFWwndconfig *wndconfig,
  ...
  if (window->monitor) {
    ...
    getFullWindowSize(window, wndconfig->width, wndconfig->height, &fullWidth,
                      &fullHeight);
  }
  ...
  window->win32.handle =
      CreateWindowExW(window->win32.dwExStyle, _GLFW_WNDCLASSNAME, wideTitle,
                      window->win32.dwStyle, xpos, ypos, fullWidth, fullHeight,
  ...
  if (_glfw_ChangeWindowMessageFilterEx) {
    _glfw_ChangeWindowMessageFilterEx(window->win32.handle, WM_DROPFILES,
                                      MSGFLT_ALLOW, NULL);
    _glfw_ChangeWindowMessageFilterEx(window->win32.handle, WM_COPYDATA,
                                      MSGFLT_ALLOW, NULL);
    _glfw_ChangeWindowMessageFilterEx(window->win32.handle, WM_COPYGLOBALDATA,
                                      MSGFLT_ALLOW, NULL);
  }
...
void _glfwPlatformSetWindowSize(_GLFWwindow *window, int width, int height) {
  if (window->monitor)
    ...
    SetWindowPos(window->win32.handle, HWND_TOP, 0, 0, fullWidth, fullHeight,
...
int _glfwPlatformCreateCursor(_GLFWcursor *cursor, const GLFWimage *image,
  ...
  for (i = 0; i < image->width * image->height; i++, target++, source += 4) {
    *target =
        (source[3] << 24) | (source[0] << 16) | (source[1] << 8) | source[2];
  }
...
void _glfwPlatformSetCursor(_GLFWwindow *window, _GLFWcursor *cursor) {
  ...
  // the following condition is not met. That way it should be safe to destroy
  // the cursor after calling glfwSetCursor(window, NULL) on all windows using
  // the cursor.

  if (_glfw.focusedWindow == window &&
      window->cursorMode == GLFW_CURSOR_NORMAL && window->win32.cursorInside) {
    if (cursor)
