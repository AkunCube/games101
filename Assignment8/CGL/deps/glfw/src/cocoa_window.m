...
static GLboolean enterFullscreenMode(_GLFWwindow *window) {
  ...
  [window->ns.object setFrame:[window->monitor->ns.screen frame] display:YES];
...
- (id)initWithGlfwWindow:(_GLFWwindow *)initWindow {
  self = [super initWithFrame:NSMakeRect(0, 0, 1, 1) pixelFormat:nil];
  if (self != nil) {
    window = initWindow;
    trackingArea = nil;
...
- (void)mouseMoved:(NSEvent *)event {
  if (window->cursorMode == GLFW_CURSOR_DISABLED) {
    _glfwInputCursorMotion(window, [event deltaX] - window->ns.warpDeltaX,
...
- (BOOL)performDragOperation:(id<NSDraggingInfo>)sender {
  ...
  const NSRect contentRect = [window->ns.view frame];
  _glfwInputCursorMotion(window, [sender draggingLocation].x,
...
void _glfwPlatformSetWindowPos(_GLFWwindow *window, int x, int y) {
  ...
  const NSRect dummyRect =
      NSMakeRect(x, transformY(y + contentRect.size.height), 0, 0);
...
void _glfwPlatformGetWindowFrameSize(_GLFWwindow *window, int *left, int *top,
  ...
  if (right)
    *right = frameRect.origin.x + frameRect.size.width - contentRect.origin.x -
...
void _glfwPlatformSetCursorPos(_GLFWwindow *window, double x, double y) {
  ...
  if (window->monitor) {
    ...
    const NSPoint globalPoint =
        [window->ns.object convertBaseToScreen:localPoint];

    CGWarpMouseCursorPosition(
        CGPointMake(globalPoint.x, transformY(globalPoint.y)));
  }
}
