...
GLboolean _glfwSetVideoMode(_GLFWmonitor *monitor, const GLFWvidmode *desired) {
  if (_glfw.x11.randr.available && !_glfw.x11.randr.monitorBroken) {
    ...
    if (!native) {
      _glfwInputError(GLFW_PLATFORM_ERROR, "X11: Monitor mode list changed");
      return GL_FALSE;
    }
  }
...
_GLFWmonitor **_glfwPlatformGetMonitors(int *count) {
  ...
  *count = 0;
  ...
  if (_glfw.x11.randr.available) {
    ...
    XRRScreenResources *sr =
        XRRGetScreenResources(_glfw.x11.display, _glfw.x11.root);
    ...
    for (i = 0; i < sr->ncrtc; i++) {
      XRRCrtcInfo *ci = XRRGetCrtcInfo(_glfw.x11.display, sr, sr->crtcs[i]);

      for (j = 0; j < ci->noutput; j++) {
        int widthMM, heightMM;
        XRROutputInfo *oi =
            XRRGetOutputInfo(_glfw.x11.display, sr, ci->outputs[j]);
  ...
  if (!monitors) {
    ...
    monitors[0] = _glfwAllocMonitor(
        "Display", DisplayWidthMM(_glfw.x11.display, _glfw.x11.screen),
        DisplayHeightMM(_glfw.x11.display, _glfw.x11.screen));
    found = 1;
  }
