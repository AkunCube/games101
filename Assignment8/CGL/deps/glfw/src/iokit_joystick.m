...
//
static void addJoystickElement(_GLFWjoystickIOKit *joystick,
  ...
  if ((elementType == kIOHIDElementTypeInput_Axis) ||
    ...
    if (elementsArray) {
      ...
      numberRef =
          CFDictionaryGetValue(elementRef, CFSTR(kIOHIDElementCookieKey));
...
//
static long getElementValue(_GLFWjoystickIOKit *joystick,
                            _GLFWjoyelement *element) {
  ...
  if (joystick && element && joystick->interface) {
    result =
        (*(joystick->interface))
            ->getElementValue(joystick->interface, element->cookie, &hidEvent);
...
//
static void removalCallback(void *target, IOReturn result, void *refcon,
                            void *sender) {
...
static void pollJoystickEvents(void) {
  ...
  for (joy = 0; joy <= GLFW_JOYSTICK_LAST; joy++) {
    ...
    for (i = 0; i < CFArrayGetCount(joystick->buttonElements); i++) {
      _GLFWjoyelement *button = (_GLFWjoyelement *)CFArrayGetValueAtIndex(
          joystick->buttonElements, i);
...
void _glfwInitJoysticks(void) {
  ...
  while ((ioHIDDeviceObject = IOIteratorNext(objectIterator))) {
    ...
    valueRef = IORegistryEntryCreateCFProperty(
        ioHIDDeviceObject, CFSTR(kIOHIDPrimaryUsagePageKey),
    ...
    valueRef = IORegistryEntryCreateCFProperty(
        ioHIDDeviceObject, CFSTR(kIOHIDPrimaryUsageKey), kCFAllocatorDefault,
        kNilOptions);
    if (valueRef) {
    ...
    result = IORegistryEntryCreateCFProperties(
        ioHIDDeviceObject, &propsRef, kCFAllocatorDefault, kNilOptions);
    ...
    result = IOCreatePlugInInterfaceForService(
        ioHIDDeviceObject, kIOHIDDeviceUserClientTypeID, kIOCFPlugInInterfaceID,
    ...
    plugInResult =
        (*ppPlugInInterface)
            ->QueryInterface(ppPlugInInterface,
    ...
    (*(joystick->interface))
        ->setRemovalCallback(joystick->interface, removalCallback, joystick,
                             joystick);

    // Get product string
    valueRef = CFDictionaryGetValue(propsRef, CFSTR(kIOHIDProductKey));
    if (valueRef) {
      CFStringGetCString(valueRef, joystick->name, sizeof(joystick->name),
    ...
    if (CFGetTypeID(valueRef) == CFArrayGetTypeID()) {
      ...
      CFArrayApplyFunction(valueRef, range, getElementsCFArrayHandler,
                           (void *)joystick);
    }
    ...
    joystick->axes =
        calloc(CFArrayGetCount(joystick->axisElements), sizeof(float));
    joystick->buttons = calloc(CFArrayGetCount(joystick->buttonElements) +
                                   CFArrayGetCount(joystick->hatElements) * 4,
                               1);
