...
static int getWindowState(_GLFWwindow *window) {
  ...
  if (_glfwGetWindowProperty(window->x11.handle, _glfw.x11.WM_STATE,
                             _glfw.x11.WM_STATE,
...
static _GLFWwindow *findWindowByHandle(Window handle) {
  ...
  if (XFindContext(_glfw.x11.display, handle, _glfw.x11.context,
                   (XPointer *)&window) != 0) {
    return NULL;
  }
...
static GLboolean createWindow(_GLFWwindow *window,
  ...
  window->x11.colormap = XCreateColormap(_glfw.x11.display, _glfw.x11.root,
  ...
  // Create the actual window
  {
    ...
    window->x11.handle =
        XCreateWindow(_glfw.x11.display, _glfw.x11.root, 0, 0, wndconfig->width,
                      wndconfig->height,
                      0,             // Border width
                      visual->depth, // Color depth
                      InputOutput, visual->visual, wamask, &wa);

    _glfwReleaseXErrorHandler();

    if (!window->x11.handle) {
      _glfwInputXError(GLFW_PLATFORM_ERROR, "X11: Failed to create window");
      return GL_FALSE;
    }
  ...
  // Declare the WM protocols supported by GLFW
  {
    ...
    if (count > 0) {
      XSetWMProtocols(_glfw.x11.display, window->x11.handle, protocols, count);
    }
  }
  ...
  if (wndconfig->floating && !wndconfig->monitor) {
    if (_glfw.x11.NET_WM_STATE && _glfw.x11.NET_WM_STATE_ABOVE) {
      changeWindowState(window, _glfw.x11.NET_WM_STATE_ABOVE,
                        _NET_WM_STATE_ADD);
    }
  }
...
static Atom writeTargetToProperty(const XSelectionRequestEvent *request) {
  int i;
  const Atom formats[] = {_glfw.x11.UTF8_STRING, _glfw.x11.COMPOUND_STRING,
                          XA_STRING};
  ...
  if (request->target == _glfw.x11.TARGETS) {
    ...
    const Atom targets[] = {_glfw.x11.TARGETS, _glfw.x11.MULTIPLE,
                            _glfw.x11.UTF8_STRING, _glfw.x11.COMPOUND_STRING,
                            XA_STRING};
  ...
  if (request->target == _glfw.x11.MULTIPLE) {
    ...
    count =
        _glfwGetWindowProperty(request->requestor, request->property,
...
static void enterFullscreenMode(_GLFWwindow *window) {
  ...
  if (_glfw.x11.NET_WM_STATE && _glfw.x11.NET_WM_STATE_FULLSCREEN) {
    ...
    changeWindowState(window, _glfw.x11.NET_WM_STATE_FULLSCREEN,
                      _NET_WM_STATE_ADD);
    ...
    XResizeWindow(_glfw.x11.display, window->x11.handle, mode.width,
                  mode.height);
  }
}
  ...
  switch (event->type) {
    case KeyPress: {
      ...
      if (window->x11.ic) {
        ...
        const int count = XwcLookupString(window->x11.ic, &event->xkey, buffer,
                                          sizeof(buffer), NULL, &status);
    ...
    case MotionNotify: {
      ...
      if (x != window->x11.warpPosX || y != window->x11.warpPosY) {
        ...
        if (window->cursorMode == GLFW_CURSOR_DISABLED) {
          ...
          _glfwInputCursorMotion(window, x - window->x11.cursorPosX,
    ...
    case ConfigureNotify: {
      ...
      if (event->xconfigure.x != window->x11.xpos ||
        ...
        _glfwInputWindowPos(window, event->xconfigure.x, event->xconfigure.y);
    ...
    case ClientMessage: {
      ...
      if (event->xclient.message_type == _glfw.x11.WM_PROTOCOLS) {
        if (_glfw.x11.WM_DELETE_WINDOW &&
          ...
          XSendEvent(_glfw.x11.display, event->xclient.window, False,
                     SubstructureNotifyMask | SubstructureRedirectMask, event);
        }
        ...
        XConvertSelection(_glfw.x11.display, _glfw.x11.XdndSelection,
                          _glfw.x11.UTF8_STRING, _glfw.x11.XdndSelection,
        ...
        XSendEvent(_glfw.x11.display, event->xclient.data.l[0], False,
                   NoEventMask, &reply);
    ...
    case SelectionNotify: {
      if (event->xselection.property) {
        ...
        char *data;
        const int result = _glfwGetWindowProperty(
...
//
unsigned long _glfwGetWindowProperty(Window window, Atom property, Atom type,
                                     unsigned char **value) {
  Atom actualType;
  int actualFormat;
...
void _glfwPlatformGetWindowPos(_GLFWwindow *window, int *xpos, int *ypos) {
  ...
  if (child) {
    int left, top;
    XTranslateCoordinates(_glfw.x11.display, window->x11.handle, child, 0, 0,
                          &left, &top, &child);
...
void _glfwPlatformGetWindowFrameSize(_GLFWwindow *window, int *left, int *top,
  ...
  if (_glfwGetWindowProperty(window->x11.handle, _glfw.x11.NET_FRAME_EXTENTS,
...
int _glfwPlatformCreateStandardCursor(_GLFWcursor *cursor, int shape) {
  cursor->x11.handle =
      XCreateFontCursor(_glfw.x11.display, translateCursorShape(shape));
  if (!cursor->x11.handle) {
    _glfwInputError(GLFW_PLATFORM_ERROR,
...
void _glfwPlatformSetClipboardString(_GLFWwindow *window, const char *string) {
  ...
  XSetSelectionOwner(_glfw.x11.display, _glfw.x11.CLIPBOARD, window->x11.handle,
                     CurrentTime);
...
const char *_glfwPlatformGetClipboardString(_GLFWwindow *window) {
  ...
  for (i = 0; i < formatCount; i++) {
    ...
    XConvertSelection(_glfw.x11.display, _glfw.x11.CLIPBOARD, formats[i],
                      _glfw.x11.GLFW_SELECTION, window->x11.handle,
                      CurrentTime);
    ...
    XDeleteProperty(_glfw.x11.display, event.xselection.requestor,
