...
static void createKeyTables(void) {
  ...
  if (_glfw.x11.xkb.available) {
    ...
    for (scancode = descr->min_key_code; scancode <= descr->max_key_code;
         scancode++) {
...
//
static Atom getSupportedAtom(Atom *supportedAtoms, unsigned long atomCount,
                             const char *atomName) {
...
static void detectEWMH(void) {
  ...
  Atom wmSupported = XInternAtom(_glfw.x11.display, "_NET_SUPPORTED", True);
  if (supportingWmCheck == None || wmSupported == None)
    return;

  // Then we look for the _NET_SUPPORTING_WM_CHECK property of the root window
  if (_glfwGetWindowProperty(_glfw.x11.root, supportingWmCheck, XA_WINDOW,
  ...
  if (_glfwGetWindowProperty(*windowFromRoot, supportingWmCheck, XA_WINDOW,
  ...
  atomCount = _glfwGetWindowProperty(_glfw.x11.root, wmSupported, XA_ATOM,
  ...
  _glfw.x11.NET_WM_FULLSCREEN_MONITORS = getSupportedAtom(
...
static GLboolean initExtensions(void) {
  ...
  _glfw.x11.vidmode.available =
      XF86VidModeQueryExtension(_glfw.x11.display, &_glfw.x11.vidmode.eventBase,
                                &_glfw.x11.vidmode.errorBase);
#endif /*_GLFW_HAS_XF86VM*/

  // Check for RandR extension
  _glfw.x11.randr.available =
      XRRQueryExtension(_glfw.x11.display, &_glfw.x11.randr.eventBase,
                        &_glfw.x11.randr.errorBase);

  if (_glfw.x11.randr.available) {
    XRRScreenResources *sr;

    if (!XRRQueryVersion(_glfw.x11.display, &_glfw.x11.randr.versionMajor,
  ...
  if (XQueryExtension(_glfw.x11.display, "XInputExtension",
    ...
    if (XIQueryVersion(_glfw.x11.display, &_glfw.x11.xi.versionMajor,
  ...
  _glfw.x11.xkb.available = XkbQueryExtension(
      ...
      &_glfw.x11.xkb.errorBase, &_glfw.x11.xkb.versionMajor,
  ...
  _glfw.x11.XdndActionCopy =
      XInternAtom(_glfw.x11.display, "XdndActionCopy", True);
  ...
  _glfw.x11.XdndSelection =
      XInternAtom(_glfw.x11.display, "XdndSelection", True);
