...
//
static GLboolean parseVersionString(int *api, int *major, int *minor,
                                    int *rev) {
  int i;
  const char *version;
...
GLboolean _glfwIsValidContextConfig(const _GLFWctxconfig *ctxconfig) {
  ...
  if (ctxconfig->api == GLFW_OPENGL_API) {
    ...
    if (ctxconfig->profile) {
      if (ctxconfig->profile != GLFW_OPENGL_CORE_PROFILE &&
        ...
        _glfwInputError(GLFW_INVALID_ENUM, "Invalid OpenGL profile");
        return GL_FALSE;
      }
  ...
  if (ctxconfig->robustness) {
    if (ctxconfig->robustness != GLFW_NO_RESET_NOTIFICATION &&
      ...
      _glfwInputError(GLFW_INVALID_ENUM, "Invalid context robustness mode");
      return GL_FALSE;
    }
  }
  ...
  if (ctxconfig->release) {
    if (ctxconfig->release != GLFW_RELEASE_BEHAVIOR_NONE &&
      ...
      _glfwInputError(GLFW_INVALID_ENUM, "Invalid context release behavior");
      return GL_FALSE;
    }
  }
...
GLboolean _glfwRefreshContextAttribs(const _GLFWctxconfig *ctxconfig) {
  ...
  if (!parseVersionString(&window->context.api, &window->context.major,
