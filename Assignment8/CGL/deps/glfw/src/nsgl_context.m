...
//
void _glfwTerminateContextAPI(void) { _glfwTerminateTLS(); }
...
int _glfwCreateContext(_GLFWwindow *window, const _GLFWctxconfig *ctxconfig,
  ...
  if (ctxconfig->major == 3 && ctxconfig->minor < 2) {
    _glfwInputError(GLFW_VERSION_UNAVAILABLE,
                    "NSGL: The targeted version of OS X does not support "
                    "OpenGL 3.0 or 3.1");
    return GL_FALSE;
  }
  ...
  if (ctxconfig->major > 2) {
    if (!ctxconfig->forward) {
      _glfwInputError(GLFW_VERSION_UNAVAILABLE,
                      "NSGL: The targeted version of OS X only supports "
                      "forward-compatible contexts for OpenGL 3.2 and above");
      return GL_FALSE;
    }

    if (ctxconfig->profile != GLFW_OPENGL_CORE_PROFILE) {
      _glfwInputError(GLFW_VERSION_UNAVAILABLE,
                      "NSGL: The targeted version of OS X only supports core "
