...
int _glfwInitContextAPI(void) {
  ...
  _glfw.egl.display =
      eglGetDisplay((EGLNativeDisplayType)_GLFW_EGL_NATIVE_DISPLAY);
  ...
  if (!eglInitialize(_glfw.egl.display, &_glfw.egl.versionMajor,
...
//
int _glfwCreateContext(_GLFWwindow *window, const _GLFWctxconfig *ctxconfig,
  ...
  // Retrieve the visual corresponding to the chosen EGL config
  {
    ...
    eglGetConfigAttrib(_glfw.egl.display, config, EGL_NATIVE_VISUAL_ID,
                       &visualID);
    ...
    if (visualID) {
      ...
      eglGetConfigAttrib(_glfw.egl.display, config, EGL_RED_SIZE, &redBits);
      eglGetConfigAttrib(_glfw.egl.display, config, EGL_GREEN_SIZE, &greenBits);
      eglGetConfigAttrib(_glfw.egl.display, config, EGL_BLUE_SIZE, &blueBits);
      eglGetConfigAttrib(_glfw.egl.display, config, EGL_ALPHA_SIZE, &alphaBits);

      info.depth = redBits + greenBits + blueBits + alphaBits;
      mask |= VisualDepthMask;
    }

    window->egl.visual = XGetVisualInfo(_glfw.x11.display, mask, &info, &count);
...
void _glfwPlatformMakeContextCurrent(_GLFWwindow *window) {
  if (window) {
    if (window->egl.surface == EGL_NO_SURFACE) {
      window->egl.surface = eglCreateWindowSurface(
          _glfw.egl.display, window->egl.config,
          (EGLNativeWindowType)_GLFW_EGL_NATIVE_WINDOW, NULL);
      if (window->egl.surface == EGL_NO_SURFACE) {
        _glfwInputError(GLFW_PLATFORM_ERROR,
    ...
    eglMakeCurrent(_glfw.egl.display, window->egl.surface, window->egl.surface,
                   window->egl.context);
  } else {
    eglMakeCurrent(_glfw.egl.display, EGL_NO_SURFACE, EGL_NO_SURFACE,
                   EGL_NO_CONTEXT);
  }
