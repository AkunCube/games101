...
int main(int argc, char **argv) {
  ...
  PrintExtensions(glXQueryExtensionsString(
      glXGetCurrentDisplay(), DefaultScreen(glXGetCurrentDisplay())));
#endif
...
void VisualInfoARB(GLContext *ctx) {
  ...
  /* WGL_NO_ACCELERATION_ARB, WGL_GENERIC_ACCELERATION_ARB,
  ...
  if (!verbose) {
    ...
    fprintf(file, " +-----+-------------------------+-----------------+--------"
                  "--+-----------------+----------+\n");
    fprintf(file, " |     |          visual         |      color      | ax dp "
                  "st |      accum      |   layer  |\n");
    fprintf(file, " |  id | tp ac gd fm db sw st ms |  sz  r  g  b  a | bf th "
                  "cl |  sz  r  g  b  a | ov un sw |\n");
    fprintf(file, " +-----+-------------------------+-----------------+--------"
                  "--+-----------------+----------+\n");
    ...
    for (i = 1; i <= maxpf; i++) {
      ...
      if (!value[0])
        continue;
      /* by default show only fully accelerated window or pbuffer capable
       * visuals */
      if (!showall && ((value[2] && !value[1]) ||
      ...
      /* acceleration */
      fprintf(file, "%s ",
              value[3] == WGL_FULL_ACCELERATION_ARB      ? "fu"
      ...
      if (WGLEW_NV_float_buffer && value[n_float])
        fprintf(file, " f ");
               ...
               value[7] == WGL_TYPE_RGBA_FLOAT_ATI)
        fprintf(file, " f ");
...
void VisualInfoGDI(GLContext *ctx) {
  ...
  if (!verbose) {
    fprintf(file, "------------------------------------------------------------"
                  "-----------------\n");
    fprintf(file, "   visual   x  bf  lv rg d st ge ge  r  g  b a  ax dp st   "
                  "accum buffs    ms \n");
    fprintf(file, " id  dep tp sp sz  l  ci b ro ne ac sz sz sz sz bf th cl  "
                  "sz  r  g  b  a ns b\n");
    fprintf(file, "------------------------------------------------------------"
                  "-----------------\n");
  ...
  } else /* verbose */
  {
    ...
    for (i = 1; i <= maxpf; i++) {
      ...
      fprintf(file, "Visual ID: %2d  depth=%d  class=%s\n", i, pfd.cDepthBits,
              pfd.cColorBits <= 8 ? "PseudoColor" : "TrueColor");
      fprintf(file,
              "    bufferSize=%d level=%d renderType=%s doubleBuffer=%ld "
              "stereo=%ld\n",
              pfd.cColorBits, pfd.bReserved,
              pfd.iPixelType == PFD_TYPE_RGBA ? "rgba" : "ci",
              pfd.dwFlags & PFD_DOUBLEBUFFER, pfd.dwFlags & PFD_STEREO);
      fprintf(file, "    generic=%d generic accelerated=%d\n",
...
void VisualInfo(GLContext *__attribute__((__unused__)) ctx) {
  /*
...
void VisualInfo(GLContext *ctx) { /* TODO */
}
...
void VisualInfo(GLContext *ctx) {
  ...
  if (fbc) {
    if (!verbose) {
      ...
      fprintf(file, " +-----+-------------------------+-----------------+------"
                    "----+-------------+-------+------+\n");
      fprintf(file, " |     |        visual           |      color      | ax "
                    "dp st |    accum    |   ms  |  cav |\n");
      fprintf(file, " |  id | tp xr cl fm db st lv xp |  sz  r  g  b  a | bf "
                    "th cl | r  g  b  a  | ns  b |  eat |\n");
      fprintf(file, " +-----+-------------------------+-----------------+------"
                    "----+-------------+-------+------+\n");
      ...
      for (i = 0; i < n_fbc; i++) {
        ...
        if (ret != Success) {
          ...
          if (GLXEW_NV_float_buffer) {
            int ret2, value2;
            ret2 = glXGetFBConfigAttrib(ctx->dpy, fbc[i],
                                        GLX_FLOAT_COMPONENTS_NV, &value2);
        ...
        /* transparency */
        ret = glXGetFBConfigAttrib(ctx->dpy, fbc[i], GLX_TRANSPARENT_TYPE,
                                   &value);
        if (Success != ret) {
          fprintf(file, " ? | ");
        } else {
        ...
        /* accum red size */
        ret =
            glXGetFBConfigAttrib(ctx->dpy, fbc[i], GLX_ACCUM_RED_SIZE, &value);
        if (Success != ret) {
          fprintf(file, " ? ");
        } else {
          if (value)
        ...
        /* accum green size */
        ret = glXGetFBConfigAttrib(ctx->dpy, fbc[i], GLX_ACCUM_GREEN_SIZE,
                                   &value);
        if (Success != ret) {
          fprintf(file, " ? ");
        } else {
        ...
        /* accum blue size */
        ret =
            glXGetFBConfigAttrib(ctx->dpy, fbc[i], GLX_ACCUM_BLUE_SIZE, &value);
        if (Success != ret) {
          fprintf(file, " ? ");
        } else {
          if (value)
        ...
        /* accum alpha size */
        ret = glXGetFBConfigAttrib(ctx->dpy, fbc[i], GLX_ACCUM_ALPHA_SIZE,
                                   &value);
        if (Success != ret) {
          fprintf(file, " ? | ");
        } else {
        ...
        ret =
            glXGetFBConfigAttrib(ctx->dpy, fbc[i], GLX_SAMPLE_BUFFERS, &value);
        if (Success != ret) {
          fprintf(file, " ? | ");
        } else {
          fprintf(file, "%2d | ", value);
...
void InitContext(GLContext *ctx) { /* TODO */
}
...
void DestroyContext(GLContext *ctx) { /* TODO */
}
...
GLboolean CreateContext(GLContext *ctx) {
  ...
  ctx->wnd = XCreateWindow(ctx->dpy, RootWindow(ctx->dpy, ctx->vi->screen), 0,
                           0, 1, 1, 0, ctx->vi->depth, InputOutput,
                           ctx->vi->visual, CWBorderPixel | CWColormap, &swa);
